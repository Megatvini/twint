...
def User(ur):
    ...
    _usr.background_image = ur['data']['user']['legacy'].get('profile_banner_url', None)

    _usr.avatar = ur['data']['user']['legacy'].get(
        'profile_image_url_https', ur['data']['user']['legacy'].get('profile_image_url_http', None)
    )

    birth_date = ur['data']['user']['legacy_extended_profile'].get('birthdate', {})

    _usr.birth_date = ','.join([f'{k}:{v}' for k, v in birth_date.items()])
